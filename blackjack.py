...
from casino.card import Card, assets_folder 
from casino.deck import Deck 
from casino.hand import Hand 
from casino.player import Player, Dealer
...
class GameWindow(tk.Tk):
    def __init__(self):
        ...
        self.quit_round_button = tk.Button(self.bottom_frame, text = 'Quit', width = 25, command = self.quit)

        self.new_game_button = tk.Button(self.bottom_frame, text = 'New Game', width = 25, command = self.quit)
    ...
    def show_gameplay_buttons(self):
        ...
        self.hit_button.pack(side = tk.LEFT, padx = (100,100))
    ...
    def show_next_round_options(self):
        ...
        self.next_round_button.pack(side = tk.LEFT , padx = (100,100))
    ...
    def on_game_over(self):
        self.hit_button.pack_forget()
        self.stick_button.pack_forget()
        self.new_game_button.pack(side = tk.LEFT, padx = (100,100))
        self.quit_button.pack(side = tk.LEFT)
...
class GameScreen(tk.Canvas):
    ...
    def setup_opening_animation(self):
            ...
            self.card_back_1 = self.create_image(self.DECK_COORDINATES, image = card_back_image)
    ...
    def display_table(self, hide_dealer = True, table_state = None):
        ...
        if has_winner and not table_state['blackjack']:
        ...
        for card_number, card_image in enumerate(dealer_card_images):
            image_pos = (self.CARD_ORIGINAL_POSITION + self.CARD_WIDTH_OFFSET * card_number, self.DEALER_CARD_HIEGHT)
            self.cards_to_deal_images.append(card_image)
            self.cards_to_deal_positions.append(image_pos)
    ...
    def play_deal_animation(self):
        ...
        target_coords = self.cards_to_deal_postions[self.cards_to_deal_pointer]
    ...
    def show_card(self):
        self.create_image(self.cards_to_deal_positions[self.cards_to_deal_pointer], image = self.cards_to_deal_images[self.cards_to_deal_pointer])
        self.update()
    ...
    def hit(self):
        ...
        card_number = len(self.game_state_player.hand.cards)
        image_pos = self.get_player_card_pos(card_number)
    ...
    def show_dealers_cards(self, table_state):
        dealer_first_card = table_state['dealer_cards'][0].get_file()
        self.create_image((self.CARD_ORIGINAL_POSITION,self.DEALER_CARD_HEIGHT), image = self.dealer_first_card)
...
class GameState:
    ...
    def show_winner_text(self, winner):
        if winner =='p':
            ...
            self.winner_text = self.crate_text(self.WINNER_TEXT_COORDS, text = 'Tie!', font = (None, 50))
        else:
    ...
    def check_for_winner(self):
        if self.plyer_has_blackjack:
            self.has_winner = 'p'
        elif self.player_is_over:
            self.has_winner = 'd'
        return self.has_winner

    def assigm_winnings(self):
